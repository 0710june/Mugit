...
// ===========================================================>
import DialogBox from "@/app/components/editor/DialogBox";
import CustomAudioBar from "@/app/components/editor/CUSTOM/audioBar/CustomAudioBar";
...
export default function Page() {
  ...
  function handleUpload(event: ChangeEvent<HTMLInputElement>) {
    ...
    // }
    // setMyMusicArray((prev) => [...prev, newfile])
  ...
  const [myMusicArray, setMyMusicArray] = useState([
    {
      file: "/sound/note.mp3",
      name: "example1",
    },
  ...
  const [fileEdit, setFileEdit] = useAtom(fileToEdit);

  useEffect(() => {
    if (fileEdit.newSources) {
      fileEdit.newSources.map((item) => {
        console.log(item);
    ...
    <>
      ...
      >
        ...
        <button
          ...
          className="absolute right-4 top-4 rounded-full bg-pointblue px-4 py-2 text-2xl italic text-white transition duration-200 hover:bg-pointyellow hover:text-pointblue"
        >
          Finish Editing
        </button>
